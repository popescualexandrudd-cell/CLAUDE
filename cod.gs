...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('🎾 MASTERS')
    .addItem('1. Construiește sistemul complet', 'buildAll')
    .addSeparator()
    .addItem('2. Setează luna de lucru', 'setWorkingMonth')
    .addItem('3. Recalculează zilele din calendar', 'refreshCalendar')
    .addSeparator()
    .addItem('4. Importă MANUAL datele din foaia brută', 'triggerManualSync')
    .addSubMenu(SpreadsheetApp.getUi().createMenu('🔄 Sincronizare Automată')
    ...
    .addSeparator()
    .addItem('5. Instrucțiuni distribuire', 'syncParentView')
    .addSeparator()
    .addSubMenu(SpreadsheetApp.getUi().createMenu('9. Notificări WhatsApp 💬')
      .addItem('LUNA NOUĂ: Trimite mesaj de salut', 'waSendMonthly')
      .addItem('MEMENTO: Trimite atenționare plată', 'waSendReminder')
    ).addToUi();
}
...
function liveSyncWorker(e) { try { var targetId = PropertiesService.getDocumentProperties().getProperty('TARGET_ID'); if (!targetId) return; var destSS = SpreadsheetApp.openById(targetId); importDataCore(destSS, true); } catch(err) { console.error(err); } }
...
function importDataCore(ss, isAuto) {
  ...
  var data = sheet.getDataRange().getValues(), backgrounds = sheet.getDataRange().getBackgrounds();
  var blocks = detectBlocks_(data, backgrounds), imported = 0, marksCount = 0, clearedSheets = {}; 
  ...
  blocks.forEach(function (b) {
    ...
    b.athletes.forEach(function (a, i) {
      ...
      a.marks.forEach(function (mark) {
        ...
        if (idx > -1 && idx < nDays) {
          sh.getRange(rIdx, LAY.fixedCols + 1 + idx).setValue(mark.code);
          searchStartIdx = idx + 1;
          marksCount++;
        }
      });
      
      if (a.zileFixe && statCol > 0) sh.getRange(rIdx, statCol).setValue(a.zileFixe);
      ...
      sh.getRange(rIdx, 3, 1, 2).setBackground(bgName).setFontColor(fgName);
    });
...
function processWhatsAppNotifications(type) {
  ...
  try {
    ...
    var data = sheet.getDataRange().getValues(), backgrounds = sheet.getDataRange().getBackgrounds();
    var blocks = detectBlocks_(data, backgrounds);
    blocks.forEach(function(b) {
...
function setWorkingMonth() { var ui = SpreadsheetApp.getUi(); var res = ui.prompt('Luna de lucru', 'Scrie luna în format AAAA-LL (Ex: 2026-09)', ui.ButtonSet.OK_CANCEL); if (res.getSelectedButton() !== ui.Button.OK) return; var v = res.getResponseText().trim(); if (!/^\d{4}-\d{2}$/.test(v)) { ui.alert('Format greșit.'); return; } PropertiesService.getDocumentProperties().setProperty('MONTH', v); SpreadsheetApp.getActiveSpreadsheet().toast('Luna a fost salvată!', 'MASTERS', 5); }
function refreshCalendar() { var ss = SpreadsheetApp.getActiveSpreadsheet(), m = getWorkingMonth_(); GROUPS.forEach(function (g) { buildGroupSheet_(ss, g, m); }); buildDashboard_(ss); SpreadsheetApp.getActiveSpreadsheet().toast('Calendarul a fost recalculat!', 'MASTERS', 5); }
...
function doGet(e) { return HtmlService.createTemplateFromFile('Index').evaluate().setTitle('Portal Părinți - Club Tenis Masters').setFaviconUrl('https://i.postimg.cc/2ythf64H/unnamed-removebg-preview-(1).png').addMetaTag('viewport', 'width=device-width, initial-scale=1').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL); }
...
function extractAthletesFromSheet_(sheet, cleanPhone, sDate, azi) {
  ...
  for (var i=0; i<blocks.length; i++) {
      for (var j=0; j<blocks[i].athletes.length; j++) {
          ...
          var aPhone = String(a.contact).replace(/\s+/g, '').replace(/^0/, '').trim();
          if (aPhone && (aPhone.indexOf(cleanPhone) > -1 || cleanPhone.indexOf(aPhone) > -1)) {
              var istoricZile = [];
              ...
              foundAthletes.push({ athleteObj: a, groupObj: matchGroup_(blocks[i].category) || { cat: blocks[i].category, time: '' }, istoric: istoricZile });
          }
      }
  }
...
function getAthleteData(phone) {
  var cleanPhone = String(phone).replace(/\s+/g, '').replace(/^0/, '').trim();
  if (!cleanPhone || cleanPhone.length < 8) return { error: 'Introduceți un număr valid (minim 9 cifre).' };
  var src; try { src = SpreadsheetApp.openById(CFG.SOURCE_ID); } catch(e) { return { error: 'Baza de date se actualizează.' }; }
  
  var allSheets = src.getSheets(), currentSheetNameTrimmed = CFG.SOURCE_SHEET.trim();
  var currentSheetDate = parseSheetDate_(currentSheetNameTrimmed) || new Date();
  var azi = new Date(), allFound = {}; 
  
  allSheets.forEach(function(sheet) {
      var shName = sheet.getName().trim();
      if (shName.indexOf('_PREZENTA') === -1) return;
      var sDate = parseSheetDate_(shName);
      if (!sDate || sDate > currentSheetDate) return; 
      
      var exts = extractAthletesFromSheet_(sheet, cleanPhone, sDate, azi);
      exts.forEach(function(ext) {
          var numeId = ext.athleteObj.numeComplet;
          if (!allFound[numeId]) allFound[numeId] = { archiveList: [], mainData: null };
          
          ext.istoric.sort(function(a, b) { return a.timestamp - b.timestamp; });
          var lunaDisplay = shName.replace('_PREZENTA', '').trim();
          
          allFound[numeId].archiveList.push({ 
             dateObj: sDate, rawExt: ext, 
             obj: { 
                 lunaNume: lunaDisplay, istoric: ext.istoric,
                 plataInfo: { status: ext.athleteObj.plata, data: ext.athleteObj.dataPlata, metoda: ext.athleteObj.metodaPlata, tarif: ext.athleteObj.tarif }
             } 
          });
          if (shName === currentSheetNameTrimmed) { allFound[numeId].mainData = ext; allFound[numeId].mainData.lunaSursa = lunaDisplay; }
      });
  });
  ...
  Object.keys(allFound).forEach(function(numeId) {
      ...
      if (athData.mainData) {
          ...
          results.push({
              ...
              istoricCurent: athData.mainData.istoric || [] // ADAUGAT PENTRU LOGICA DE ABSENTE
          });
      }
  });
  
  if (results.length === 0) return { error: 'Numărul nu a fost găsit în baza de date.' };
  results.sort(function(a, b) { return a.nume.localeCompare(b.nume); }); // Afisare ordonata alfabetic
...
function formatZileFixe_(txt) { var s = String(txt || '').toUpperCase().replace(/(\d{1,2})[.:h](\d{2})\s*[-–]\s*(\d{1,2})[.:h](\d{2})/g, '').trim(); if (!s) return ''; var map = { 'LUNI':'Luni','MARTI':'Marți','MARȚI':'Marți','MIERCURI':'Miercuri','JOI':'Joi','VINERI':'Vineri','SAMBATA':'Sâmbătă','SÂMBĂTĂ':'Sâmbătă','L':'Luni','MA':'Marți','MI':'Miercuri','J':'Joi','V':'Vineri' }; var out = []; s.split(/[\/,\-–|+]+/).forEach(function(p) { p = p.trim(); if (map[p] && out.indexOf(map[p]) === -1) out.push(map[p]); }); return out.join(', '); }
